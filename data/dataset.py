...
from data import data_list, IGNORE_INDEX
from data.rope2d import get_rope_index_2
from Arguments import DataArguments
from PIL import Image
from data.process import preprocess_qwen_2_visual

import numpy as np
from Arguments import rank0_print
from utils import pad_and_cat
...
#自定义datacollator
@dataclass
class CustomizedDataCollatorForSupervisedDataset(object):
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args):
        ...
        dataset_list = data_list(data_args.data_names)
    ...
    def process_image_unified(self, image_file):
        """
        类似图像分块，从时间和空间上进行
        grid_t,temporal_patch_size,channel,grid_h // merge_size,merge_size,patch_size,grid_w // merge_size,merge_size,patch_size,
        ===>
...
class FlattenedDataCollatorForSupervisedDataset(DataCollatorForSupervisedDataset):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        print('attn',cumsum_seq_lens,position_ids.shape,input_ids.shape)
        batch = dict(
        ...
        batch["pixel_values_videos"] = concat_videos
        batch["video_grid_thw"] = video_grid_thw
        print(local_rank, batch["pixel_values"].shape, batch["input_ids"].shape,6666 ,batch["image_grid_thw"].shape,batch["image_grid_thw"])
        return batch
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    # data_collator = CustomizedDataCollatorForSupervisedDataset(tokenizer=tokenizer)
